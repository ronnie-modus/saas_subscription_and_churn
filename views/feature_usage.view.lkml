view: feature_usage {
  ...
  dimension: dynamic_usage_date {
    type:        string
    label:       "Usage Date (Dynamic)"
    description: "Usage date truncated to the chosen granularity."
    sql:
      {% if date_granularity._parameter_value == "'day'" %}
        CAST(SAFE_CAST(${TABLE}.usage_date AS DATE) AS STRING)
      {% elsif date_granularity._parameter_value == "'week'" %}
        CAST(DATE_TRUNC(SAFE_CAST(${TABLE}.usage_date AS DATE), WEEK) AS STRING)
      {% elsif date_granularity._parameter_value == "'month'" %}
        FORMAT_DATE('%Y-%m', SAFE_CAST(${TABLE}.usage_date AS DATE))
      {% elsif date_granularity._parameter_value == "'quarter'" %}
        CONCAT(CAST(EXTRACT(YEAR FROM SAFE_CAST(${TABLE}.usage_date AS DATE)) AS STRING), '-Q',
               CAST(EXTRACT(QUARTER FROM SAFE_CAST(${TABLE}.usage_date AS DATE)) AS STRING))
      {% elsif date_granularity._parameter_value == "'year'" %}
        CAST(EXTRACT(YEAR FROM SAFE_CAST(${TABLE}.usage_date AS DATE)) AS STRING)
      {% else %}
        FORMAT_DATE('%Y-%m', SAFE_CAST(${TABLE}.usage_date AS DATE))
      {% endif %} ;;
  }
